...
public void setup(){
    ...
    people[0] = new person ( 0 , new PVector( width / 4 , height/4 ) , color( 252 , 0 , 98 ) , 38 , 40 , 37 , 39 , 17 ); 
    // 38 -- Up
    // 40 -- Down
    // 37 -- Left
    // 39 -- Right
    ...
    people[1] = new person ( 1 , new PVector( (0.75f) * width ,  height / 4 ) , color( 0 , 129 , 255 ) , 87 , 83 , 65 , 68 , 81 ); 
    // 87 -- W
    // 83 -- S
    // 65 -- A
    // 68 -- D
    ...
    people[2] = new person ( 2 , new PVector( (0.75f) * width , (0.75f) * height ) , color( 255 , 145 , 0 ) , 89 , 72 , 71 , 74 , 84 ); 
    // 89 -- Y
    // 72 -- H
    // 71 -- G
    // 74 -- J
    ...
    people[3] = new person ( 3 , new PVector( width / 4  , (0.75f) * height ) , color( 58 , 255 , 0 ) , 80 , 59 , 76 , 222 , 79 ); 
    // 80 -- P
    // 186 -- ;
    // 76 -- l
    // 222 -- '
    ...
    public void run( ArrayList<person> artists_ ){
        ...
        if ( trail.size() > 0){
            for ( int i = 0 ; i < trail.size() ; i ++ ){//for every trail
                ...
                else if ( new_path.size() > 0 || new_path.size() >= path_size ) { 
...
public person[] arraylist_to_list( ArrayList<person> als ){
    ...
    static public void main(String[] passedArgs) {
        String[] appletArgs = new String[] { "--present", "--window-color=#CBCBCB", "--stop-color=#cccccc", "Tile_Matrix" };
        if (passedArgs != null) {
